...
void Wire2(int index, int direction, int num)
{
 	for(int i = 0; i < num; i++){
		...
		if (!dict1.ContainsKey(current[0] + "," + current[1] )) 
		{
			if (!dict2.ContainsKey(current[0] + "," + current[1] ))
				dict2.Add(current[0] + "," + current[1] , new int[] { 2, count });
		}
		else
		{
			if (!dict2.ContainsKey(current[0] + "," + current[1] ))
			{
				dict2.Add(current[0] + "," + current[1] , new int[] { 3, count });
				manDis.Add(Math.Abs(current[0]) + Math.Abs(current[1]));
				intersections.Add(count + dict1[current[0] + "," + current[1]][1]);
			}
		}
	}
}
...
{
  var ch = (char)d[0];
  var num = Int32.Parse(d.Remove(0,1));
	switch (ch)
		{
		case 'U': Wire1(0, 1, num); break;
		case 'L': Wire1(1, -1, num); break;
		case 'R': Wire1(1, 1, num); break;
		case 'D': Wire1(0, -1, num); break;
		}
}

current = new int[2]{0,0};
count = 0;
...
foreach(var d in directions2)
{
  ...
  var num = Int32.Parse(d.Remove(0,1));
	switch (ch)
		{
		case 'U': Wire2(0, 1, num); break;
		case 'L': Wire2(1, -1, num); break;
		case 'R': Wire2(1, 1, num); break;
		case 'D': Wire2(0, -1, num); break;
		}
}
